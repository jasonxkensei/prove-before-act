...
# 审计报告: provebeforeact.com/incident/{钱包地址}/{why_proof_id}
```
...
# 返回: {"proof_id": "...", "verify_url": "provebeforeact.com/proof/..."}
```
...
r = requests.post("https://provebeforeact.com/api/proof",
...
```
https://provebeforeact.com/incident/{钱包地址}/{proof_id}
```

或通过API程序化获取：

```bash
curl https://provebeforeact.com/api/agents/{钱包地址}/incident-report?proof_id={uuid}
```
...
| Vercel AI | `@prove-before-act/sdk` NPM 包 |
...
或访问 [provebeforeact.com/zh](https://provebeforeact.com/zh) 通过界面直接获取。
...
Prove Before Act 提供的链上存证记录可作为合规审计的技术支撑，适用于：

- 《生成式人工智能服务管理暂行办法》操作记录要求
- 《互联网信息服务算法推荐管理规定》算法决策透明度要求
- 企业内部AI治理审计需求

> **注意**：Prove Before Act提供技术存证工具，不提供法律合规建议。具体合规方案请咨询您的法律顾问。

---

## 链接

- 官网（中文）：[provebeforeact.com/zh](https://provebeforeact.com/zh)
- 开发文档：[provebeforeact.com/docs](https://provebeforeact.com/docs)
...
*Prove Before Act — 每次智能体操作，均可链上证明。*
