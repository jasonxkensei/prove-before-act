...
2. Prove Before Act responds with `402 Payment Required` and a JSON body containing payment requirements
...
6. Prove Before Act verifies and settles the payment, then processes the certification
...
```json
{
  ...
  "accepts": [
    {
      "scheme": "exact",
      "price": "<current live price — see /api/pricing>",
      "network": "eip155:8453",
      "payTo": "0x...",
...
| Endpoint | Price |
|:---|:---|
| `POST /api/proof` | current live price (see /api/pricing) |
| `POST /api/batch` | current live price per file (see /api/pricing) |
...
# Body: {"x402Version":1,"accepts":[{"scheme":"exact","price":"<current live price>","network":"eip155:8453","payTo":"0x..."}],...}
...
- **Composable** -- any x402-compatible agent (Conway, OpenClaw, custom) can use Prove Before Act
...
Any Conway automaton or OpenClaw agent with x402 support can certify via Prove Before Act without prior registration. The agent just needs:
...
No MultiversX wallet is required on the agent side -- Prove Before Act handles the blockchain anchoring.
